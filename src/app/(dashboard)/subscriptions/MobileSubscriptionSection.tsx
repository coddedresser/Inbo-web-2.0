...
export default function MobileSubscriptionSection() {
  ...
  const publicationCount = tab === "active" ? active.length : inactive.length;
  ...
  return (
    <div className="min-h-screen bg-[#F5F6FA]">
      ...
      <div className="flex items-center h-[60px] px-4 bg-[#F5F6FA]  border-[#EAECF0]">
        ...
        <p className="text-[18px] font-semibold flex-1 text-center mr-6">Subscription</p>
      </div>
      <div className="mt-2
          bg-white
          rounded-2xl
      ...
      <div className="flex justify-between items-center px-4 mt-5">
        <p className="text-[16px] font-medium">{publicationCount} Publication</p>
        ...
        <div className="flex rounded-full bg-white border border-[#D8DDE3] p-1">
          <button
            ...
            className={`px-4 py-1 text-[14px] font-medium rounded-full ${
              tab === "active" ? "bg-[#D0D4DB] text-[#0C0D0E]" : "text-[#6F7680]"
            }`}
          >
            Active
          </button>
          ...
          <button
            ...
            className={`px-4 py-1 text-[14px] font-medium rounded-full ${
              tab === "inactive" ? "bg-[#D0D4DB] text-[#0C0D0E]" : "text-[#6F7680]"
            }`}
          >
            Inactive
          </button>
        ...
        {(tab === "active" ? active : inactive).map((p) => (
          ...
          >
            ...
            <div className="flex flex-col items-start">
              <p className="text-[15px] font-semibold text-left">{p.name}</p>
...
/* ============================================================
   PUBLISHER DETAIL — New Pixel-Perfect Layout
============================================================ */
 ...
 return (
  <div className="min-h-screen bg-[#F5F6FA]">
    ...
    >
      {/* ================= TOP CARD ================= */}
      <div className="p-5">
        {/* ROW 1 — ICON + TEXT BLOCK */}
        <div className="flex gap-4">
          <div className="w-[62px] h-[62px] rounded-full overflow-hidden flex-shrink-0">
            <img src={state.logo} className="w-full h-full object-cover" />
          </div>

          <div className="flex flex-col flex-1">
            <p className="text-[17px] font-bold">{state.name}</p>

            <p className="text-[14px] text-[#6F7680] mt-1">
              {state.description}
            </p>

            <p className="text-[14px] mt-2">
              <span className="font-medium">First mail: </span>
              <span className="text-[#6F7680]">{state.firstMail}</span>
            </p>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center mt-4">
          <button className="text-[15px] text-gray-500 font-semibold">
            Unsubscribe
          </button>
      ...
      <div className="bg-[#F3F4F6] mx-4 mb-4 rounded-2xl p-5">
        <p className="text-[18px] font-semibold mb-4">Your Engagement</p>
        ...
        <div className="flex justify-between mb-4">
          ...
          <div>
            ...
            <p className="text-[24px] font-bold">{readPercentage}%</p>
          </div>
        </div>
      ...
      <div className="px-4 pb-6">
        <div className="flex justify-between items-center">
          <p className="text-[16px] font-semibold">Recent Issues</p>
          <FilterButton />
        </div>

        <div className="mt-5 space-y-4">
          {state.newsletters.map((n) => (
            <MobileInboxCard key={n.id} {...n} slug={n.id} />
          ))}
        </div>
      </div>
    </div>
